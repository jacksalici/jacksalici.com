---
title: "Flying Bear Ghost 4S"
pubDate: 2026-01-03
slug: "fb-ghost4s"
description: "Stampante 3D FDM perfettamente funzionante in vendita."
gallery:
  - src: /uploads/ghost4s/p2.jpg
    caption: "Vista frontale della Flying Bear Ghost 4S"
  - src: /uploads/ghost4s/p1.jpg
    caption: "Dettaglio del piano di stampa"
  - src: /uploads/ghost4s/p5.jpg
    caption: "Sensore e meccanismo di alimentazione"
  - src: /uploads/ghost4s/p3.jpg
    caption: "Schermo touchscreen intuitivo"
  - src: /uploads/ghost4s/p4.jpg
    caption: "Struttura in metallo"
  - src: /uploads/ghost4s/m1.jpg
    caption: "Prova di stampa 1"
  - src: /uploads/ghost4s/m2.jpg
    caption: "Prova di stampa 2"
  - src: /uploads/ghost4s/m3.jpg
    caption: "Prova di stampa 3"
---
## Overview

La Flying Bear Ghost 4S è una stampante FDM del 2019, che offre un volume di stampa di 255mm x 210mm x 210mm in un ingombro compatto di 392mm x 342mm x 420mm. La struttura completamente in metallo garantisce un'esperienza di stampa affidabile e un corpo robusto che non mi ha deluso.

Ho utilizzato questa stampante per vari progetti, inclusa la stampa di parti funzionali, prototipi e modelli artistici. Ha costantemente fornito stampe di alta qualità con buona adesione dei layer e dettagli.

Dopo esattamente 5.5 anni di onorato servizio, vendo per passaggio ad una stampante con altre funzionalità.
Ore di stampa totali: ~500 ore. Ancora perfettamente funzionante.

| | |
|---|---|
| Dimensioni | 392mm x 342mm x 420mm |
| Volume di Stampa | 255mm x 210mm x 210mm |
| Tipo di Stampante | FDM |
| Struttura | Metallo |
| Meccanismo di Movimento | Cartesian |
| Peso | 13.5kg |
| Risoluzione dei Layer | 0.05–0.3mm |
| Diametro Filamento | 1.75mm |
| Diametro Ugello | 0.4mm |
| Velocità di Stampa | 20–150mm/s |
| Velocità di Movimento | 120mm/s |
| Temperatura Massima Ugello | 260°C |
| Temperatura Massima Piatto | 110°C |
| Filamenti Supportati | PLA, PET, PETG, ABS, ecc. |
| Bed Leveling | Manuale |
| Supporto Wi-Fi | Sì |

## Condizioni

La stampante funziona perfettamente e non necessita di alcun upgrade immediato. Negli anni ho aggiornato il tubo bowden con uno in PTFE Capricorn per un migliore controllo del filamento e un attrito ridotto. Inoltre, ho installato un estrusore _dual gear_ per migliorare l'alimentazione del filamento e ridurre le possibilità di slittamento. 

## Costo

150, passaggio a mano zona Modena.


## Contatti
Per ulteriori informazioni, non esitate a contattarmi tramite la pagina [Contatti](/contacts).
